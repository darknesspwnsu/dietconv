...
def plot_size(rows: list[dict[str, str]], output_dir: Path) -> None:
    ...
    for backend, backend_rows in by_backend.items():
        ...
        axes[0].plot(xs, peak, marker="o", linewidth=2, color=COLORS[backend], label=LABELS[backend])
        axes[1].plot(xs, delta, marker="o", linewidth=2, color=COLORS[backend], label=LABELS[backend])

    axes[0].set_title("PyTorch peak process RSS vs input size (isolated process)")
    axes[0].set_xlabel("Input height/width")
    axes[0].set_ylabel("Peak RSS (MiB)")
    axes[0].legend()

    axes[1].set_title("PyTorch RSS delta vs input size (isolated process)")
    axes[1].set_xlabel("Input height/width")
    axes[1].set_ylabel("Peak RSS delta (MiB)")
    axes[1].set_yscale("log")
    axes[1].legend()

    fig.savefig(output_dir / "torch_memory_size_scaling.png", dpi=160)
...
def plot_threads(rows: list[dict[str, str]], output_dir: Path) -> None:
    ...
    for axis, problem in zip(axes, problems):
        ...
        for backend, backend_rows in by_backend.items():
            ...
            ys = [float(row["rss_delta_mib"]) for row in backend_rows]
            axis.plot(xs, ys, marker="o", linewidth=2, color=COLORS[backend], label=LABELS[backend])
        axis.set_title(f"PyTorch RSS delta vs threads: {problem}")
        axis.set_xlabel("Threads")
        axis.set_ylabel("Peak RSS delta (MiB)")
        axis.set_xticks([1, 2, 4, 8])
        axis.legend()
    fig.savefig(output_dir / "torch_memory_thread_delta.png", dpi=160)


def main() -> None:
    parser = argparse.ArgumentParser(description="Plot torch process-memory benchmark outputs.")
    parser.add_argument("--results-dir", type=Path, default=ROOT / "results")
    args = parser.parse_args()

    size_rows = read_rows(args.results_dir / "torch_memory_size_scaling.csv")
    thread_rows = read_rows(args.results_dir / "torch_memory_thread_scaling.csv")
    ...
    print(f"Wrote torch memory benchmark plots to {args.results_dir}")
